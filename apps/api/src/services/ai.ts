...
const NOTE_PROMPT_RULES = `note — максимально полное описание операции. Не выдумывай поля, которых нет на фото/в тексте/в CSV.
Самое важное в начале (поиск идёт по note). Не дублируй сумму, дату и категорию приложения — они в отдельных полях.
Если поля нет — пропусти, не пиши «неизвестно». Разделитель: « · ». Не длиннее ${TRANSACTION_NOTE_MAX_LENGTH} символов.
Порядок фрагментов (только то, что видно):
1) суть: мерчант / ФИО или телефон контрагента / назначение платежа;
2) банк: Сбер, Тинькофф, Альфа, ВТБ, Озон Банк и т.д. (даже если банк уже в accountHint — продублируй в note);
3) кто сделал перевод (только для P2P/переводов, не для покупок в магазине):
   исходящий → «я отправил»; входящий от другого → «мне прислали» + имя/телефон отправителя; между своими счетами → «между своими»;
4) остальное, если есть: маска карты, тип счёта, комментарий к переводу, MCC, категория банка, город/терминал, номер операции, статус.
Примеры: «Пятёрочка · Сбер · *1234»; «Перевод Анне И. · Тинькофф · я отправил · +7900… · за обед»; «Перевод от Ивана · Сбер · мне прислали».`;
